#!/usr/bin/env python
"""
Run a small synthetic recovery test with the local ck_all integrated grid.

The script creates magnitude photometry in the project format:
    photband mag mag_err system mag_type mag_zp_offset
then fits it through the normal setup/get_observations/fit_sed path.
"""
...
OUTDIR = ROOT / 'sed_fit_tests' / 'synthetic_recovery_ck_all'
...
def _write_setup(path, photometry_path):
    setup = {
        'objectname': 'synthetic_ck_all',
        'photometryfile': str(photometry_path),
        'photband_exclude': [],
        'pnames': ['teff', 'logg', 'feh', 'rad', 'distance', 'av'],
        'limits': [
            [7000.0, 9000.0],
            [3.5, 4.5],
            [-1.0, 0.0],
        ...
        # Broadband SEDs constrain rad/distance, not both independently.
        # A distance prior makes this synthetic recovery well posed.
        'priors': {
            'distance': [TRUTH['distance'], 40.0],
        },
        'grids': ['ck_all'],
        ...
        'resultfile': str(OUTDIR / 'synthetic_ck_all_results.csv'),
        'datafile': str(OUTDIR / 'synthetic_ck_all_samples.fits'),
        'plot1': {
            'type': 'sed_fit',
            'result': 'pc',
            'path': str(OUTDIR / 'synthetic_ck_all_sed.png'),
        },
        'plot2': {
            'type': 'distribution',
            'show_best': True,
            'path': str(OUTDIR / 'synthetic_ck_all_distribution.png'),
            'parameters': ['teff', 'logg', 'feh', 'rad', 'distance', 'av'],
        },
    }
...
def _make_photometry(path):
    ...
    observed = flux * (1.0 + rng.normal(0.0, 0.01, size=len(flux)))
    ...
    ascii.write(table, OUTDIR / 'synthetic_ck_all_photometry_with_truth.dat', overwrite=True)
    return table


def _write_recovery_summary(path, results):
    rows = []
    for name in ['teff', 'logg', 'feh', 'rad', 'distance', 'av']:
...
def main_cli():
    ...
    photometry_path = OUTDIR / 'synthetic_ck_all.phot'
    setup_path = OUTDIR / 'synthetic_ck_all_setup.yaml'
    ...
    summary = _write_recovery_summary(OUTDIR / 'synthetic_ck_all_recovery_summary.dat', results)
    print(summary)
